# Créé par Amaury LIME, le 02/01/2023 en Python 3.7
...
fond = pygame.image.load("").convert() # chercher les images et les sauvegarder dans dossier
'''
'''
côté interface :
- temps
...
fenetre = pygame.display.set_mode((1000,800)) # taille de la fenêtre
run = True
...
while run:
    ...
    for x in range(nb_cases_cote):
        for y in range(nb_cases_cote): # on parcoure les 2 dimensions
            ...
            pygame.draw.rect(fenetre, [255]*3, [x*taille_case, y*taille_case, taille_case, taille_case], 1) # dessin du rect avec la méthode que je t'avais marqué
